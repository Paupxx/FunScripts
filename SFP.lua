...
local getRandomBot = function()
    local Bots = {}
    for i,v in pairs(scriptedObjects:GetChildren()) do
        if v.Name == "AI" and v.Humanoid and v.Humanoid.Health ~= 0 and (Player.Character.HumanoidRootPart.Position-v.HumanoidRootPart.Position).Magnitude <= 100 and v:FindFirstChildOfClass("Tool") and isValidBotPosition(v) == true then
            table.insert(Bots, v)
        end
    end
    local BotsAmount = #Bots
    local BotChosen =  Bots[math.random(1,BotsAmount)]
    return BotChosen
end
